...
const tweetEmbed = (opUsername, opTag, opPFP, tweetURL, tweetContent, tweetLikes, tweetRetweets, tweetReplies, rUsername, rPFP, rContent, image) => {
    ...
    const footerContent = `Posted by ${rUsername} ${rContent.length > 0 ? `- "${rContent}"` : ''}`
...
client.on("messageCreate", async message => {
  ...
  if (matches && matches[3]) {
    ...
    let videoURLS = data.media_extended.filter((media) => media.type == 'video').map((video) => video.url);
    let imageURLS = data.media_extended.filter((media) => media.type == 'image').map((image) => image.url);
    let gifURLS = data.media_extended.filter((media) => media.type == 'gif').map((gif) => gif.url);
    let mainEmbed = tweetEmbed(
        data.user_name, 
        ...
        data.retweets,
        data.replies,
        nickname ? `${nickname} (${message.author.displayName})` : message.author.displayName, 
        avatar, 
        content,
        imageURLS[0] 
    );
    let imageEmbeds = imageURLS.slice(1).map((imageURL) => imageEmbed(`https://twitter.com/${data.user_screen_name}`, imageURL));
